...
The Fee Router is the infrastructure-level contract that **redirects pETH-denominated protocol revenue back into the ecosystem**.

Its role is to collect pETH fees, process them through the bonding curve and send the resulting pETH rewards onward to pETH flows. pAsset flows, such as USDp minter interest, do not pass through the Fee Router and are distributed by their own market flow logic.
...
Passing protocol revenue through the bonding curve enables the same value to strengthen the pETH floor before becoming rewards to be distributed throughout the ecosystem. And by the time those rewards reach their recipients, both the floor price and the market price of pETH have already benefited from protocol activity.
