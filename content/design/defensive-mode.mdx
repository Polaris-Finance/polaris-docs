...
description: "Defensive Mode is the first protective state a pAsset market enters when its collateralization becomes stressed."
---
...
In practice this state should be rare. The [Protocol Safety Rate](/design/interest-rates#protocol-safety-rate) makes leverage progressively more expensive as collateral health weakens, discouraging positions from pushing a market toward Defensive Mode in the first place.

Each [pAsset market](/architecture/passet-markets) has its own thresholds. Stress affecting one market does not automatically place every other market into the same state, so [USDp](/core-assets/usdp), [GOLDp](/core-assets/goldp) and future pAssets remain independent from one another.
...
If a market's collateralization keeps weakening past full backing, it enters [Recovery Mode](/design/recovery-mode).

---

<br />

**Note:** Defensive Mode is a feature of individual pAsset markets. It does not affect the [bonding curve](/architecture/bonding-curve), which continues to function normally regardless of the state of any pAsset market.
