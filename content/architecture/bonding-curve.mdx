...
Bonding curves have been used across crypto for years, particularly in DEXs and launchpads, where they are mainly used to price and exchange assets. Polaris uses a bonding curve for a different purpose: creating a shared collateral asset for decentralized currencies and synthetic assets across the protocol.
...
As such, value generated anywhere in Polaris accumulates around a single collateral asset without sacrificing scalability, enabling new financial primitives to build on pETH while strengthening the same foundation that supports the whole protocol.
