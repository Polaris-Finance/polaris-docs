...
Nothing here is financial advice. Polaris is currently in the phase shown on [Launch Status](/launch-status).
...
Until audit reports appear on [Audits And Security](/resources/audits-security), treat the protocol as unaudited.
...
Current testnet and production address status lives on [Contracts And Addresses](/resources/testnet#contracts-and-addresses).
...
See [Peg Defense](/redemptions-liquidations).
...
**Mitigation:** Monitor your position, keep an LTV buffer, and understand the current rates before issuing pAssets. See [Interest Rates](/minting/interest-rates).
...
Depositing pAssets into an [Earn Vault](/yield) can earn protocol-native yield, but the vault is the system's first-loss liquidation backstop.
...
Size deposits accordingly and read [Earn Vaults](/yield) before depositing.
...
For the reasoning behind these trade-offs, read [Why Polaris](/why-polaris).

For definitions, see the [Glossary](/resources).
