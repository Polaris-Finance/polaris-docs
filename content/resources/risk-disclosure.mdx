...
description: "A catalog of the risks of using Polaris, each paired with the mechanism that mitigates it without implying any risk is eliminated."
---
...
Nothing here is financial advice. Polaris is currently in the phase shown on [Launch Status](/launch-status). Testnet assets have no monetary value, and production use is not live unless Launch Status says it is.
...
Final audit reports are not published in these docs yet. Until reports appear on [Audits And Security](/resources/audits-security), treat the protocol as unaudited.
...
Depositing pAssets into an [Earn Vault](/yield) is the primary way to earn pAsset yield, but the vault is the system's first-loss backstop.
...
See [Stewardship](/stewardship) and [Parameters](/resources/testnet#parameters).
...
The current status checklist lives on [Launch Status](/launch-status) and [Public Testnet 1](/resources/testnet).

For the reasoning behind these trade-offs, read [Why Polaris](/why-polaris). For definitions, see the [Glossary](/resources).
