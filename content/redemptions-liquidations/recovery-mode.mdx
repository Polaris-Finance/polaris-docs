...
Recovery Mode thresholds and current per-pAsset values live on [Parameters](/resources/testnet#parameters). This page explains behavior, not current numbers.
...
The principle: actions that strengthen backing stay open; actions that thin backing face a higher bar. In practice, a collateral withdrawal or new issuance on its own always raises LTV, so in Recovery Mode it goes through only as part of a combined adjustment that offsets it - adding collateral or repaying debt in the same transaction. Current trigger and action values are published on [Parameters](/resources/testnet#parameters).
...
See [Managing Your Position](/minting/managing-your-trove) for the mechanics, and [Liquidations](/redemptions-liquidations/liquidations) for what happens to a position that exceeds the max LTV.
