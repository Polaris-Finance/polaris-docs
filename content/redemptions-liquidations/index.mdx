...
> **Actionability:** Redemptions are testnet-actionable only on Sepolia using test assets and current manifests. Production redemptions are not mainnet-actionable until [Polaris Launch Status](/launch-status) publishes the official app, production addresses, final fee formula, and audit status.
...
Redemption fees, decay factors, and current network values live on [Parameters](/resources/testnet#parameters). This page explains the mechanism; the parameter page owns the numbers.
...
- [Recovery Mode](/redemptions-liquidations/recovery-mode) for the system-wide defensive state.
- [Risk Disclosure](/resources/risk-disclosure) for the full risk model.
