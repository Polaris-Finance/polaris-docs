...
That floor access comes through [Reserve Loans](/architecture/reserve-loans). A user can borrow ETH against fpETH up to the floor value represented by that fpETH, while the fpETH remains in the Reserve Loan position. This does not burn fpETH and does not require any additional accounting for the vpETH component.
