---
title: "vpETH"
description: "Every time pETH trades above its mathematically-derived floor, the difference represents its market premium."
---
...
<img
  src="/infographics/vpeth.jpg"
  alt="How vpETH represents the changing premium share between the pETH spot price and floor price along the bonding curve."
  width="1600"
  height="990"
/>
...
Because vpETH represents the portion of pETH that trades above its bonding curve floor, it can also serve as a useful hedging tool. For example, users holding overcollateralized debt positions on Polaris may choose to short vpETH to reduce their exposure to changes in the premium while continuing to hold the underlying pETH position.

This effectively leaves the user exposed only to the ETH/USD price, which also can be hedged on an external venue to make the position delta-neutral, while preserving exposure to growth in the pETH floor.

Because the position and hedge values can change, maintaining neutrality may require rebalancing, while the strategy remains exposed to risks external to Polaris.
