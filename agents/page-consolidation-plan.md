...
Prepared 2026-06-08. **Analysis only — nothing implemented.**

## The problem

There are **69 navigable content pages**. The bloat is not from any one section
...
2. **`paths/` goal-router** (7): "Choose Your Path" — a goal→link table page
...
3. **`using-app/` app-label router** (10): one hub + per-tab pages. `using-app/guide`
   is a near-duplicate of `paths/index` (same "choose by goal" table).
...
| 8–11 | `paths/borrow-passets`, `paths/earn-yield`, `paths/hold-use-peth`, `paths/convert-lock-polar` | Fold all four into the existing `paths/index` table (already links to the deeper concept pages). | They are link-list stubs (35–40 lines) duplicating the index's job. |
| 12 | `using-app/guide` | Delete; its "choose by goal" table is the same as `paths/index`. Keep one router. | Direct duplicate. |

After Tier A: keep `paths/index` (goal router) + `using-app/index` (app-label
router) + `paths/safety-verification` + `paths/develop` as the surviving routers.
...
| 15 | `polar/participate-in-conversion` | `polar/conversion-auctions` (mechanism + how-to on one page). |
| 16 | `redemptions-liquidations/recovery-mode` | `redemptions-liquidations/liquidations` as a section. |
| 17 | `minting/minting-passets` | `minting/index` (overview already covers minting concept). |
| 18 | `stewardship/flows` | `stewardship/fee-router` (flows = fee-router detail; also fixes the /flows mobile overflow defect). |
| 19 | `polar/polar-token` | `polar/index` (overview + token basics together). |
| 20 | `peth/floor-price` | `peth/bonding-curve` (floor price is a bonding-curve property). |
...
- All concept reference pages with real depth: `minting/open-a-trove`,
  `minting/managing-your-trove`, `peth/split`, `peth/bonding-curve`,
...
- `paths/safety-verification` — top-33 wants this *promoted*, not cut.

## Execution sequence (when approved)

1. Branch off `main`.
2. **Tier A first** — delete 7 alias files + 4 paths children + using-app/guide;
   expand `paths/index` to absorb the 4 goal rows; remove their `_meta.js` keys.
...
4. Update every inbound link: `grep -rn "/borrow\b\|/paths/borrow-passets\|using-app/guide\| ...` across `content/` before deleting each page.
